...
export default function ApproveMember({ member }: { member: Member }) {
  const router = useRouter();

  const approveMemberHandle = async (id: number) => {
    try {
      const res = await setStatusMember(id, "approved");
      if (res.success) {
        ...
        router.push("/admin/members?tab=all");
      }
    } catch (err: unknown) {
  ...
  return (
    <AlertDialog>
      <AlertDialogTrigger asChild>
        <Button
          size="sm"
          className="bg-techtona-2 text-techtona-1 font-semibold hover:bg-techtona-5 cursor-pointer"
        >
          <Check />
          Approve
        </Button>
      ...
      <AlertDialogContent>
        <AlertDialogHeader className="mb-4">
          ...
          <AlertDialogDescription className="text-center text-zinc-700">
            This action will Approve the member. Approve
        ...
        <AlertDialogFooter className="md:justify-center">
          ...
          <AlertDialogAction asChild>
            <button
              ...
              onClick={() => approveMemberHandle(Number(member.id_member))}
            >
