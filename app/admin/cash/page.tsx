...
export default function Cash() {
	...
	return (
		...
		<div className="w-full pb-6 md:pb-10">
			<h1 className="hidden md:block text-xl md:text-2xl font-bold mb-3">Cash</h1>
			<Breadcrumb />
		</div>
		<h2 className="pb-3">
		...
		<div className="text-sm grid grid-cols-2 md:grid-cols-1 lg:grid-cols-2 2xl:grid-cols-4 gap-4">
			{[
			{ label: 'Income', value: '500.000', Icon: ArrowUpCircleIcon, iconColor: 'text-green-500' },
			{ label: 'Expense', value: '100.000', Icon: ArrowDownCircleIcon, iconColor: 'text-red-500' },
			{ label: 'Net Income', value: '400.000', isDoubleIcon: true },
			{ label: 'Balance', value: '5.200.000', isDark: true, Icon: BanknotesIconSolid }
			].map((box, i) => (
			<div key={i} className={`p-5 shadow-sm rounded-xl ${box.isDark ? 'bg-gray-800 text-white' : 'bg-white'}`}>
				...
				<div className="absolute right-0 top-0 md:top-1/2 md:-translate-y-1/2">
					...
					<>
						...
						<span className="absolute bottom-0 right-0 p-0.5 rounded-full bg-white">
							<box.Icon className={`size-3 md:size-5 ${box.iconColor || ''}`} />
						</span>
						)}
					</>
					) : (
