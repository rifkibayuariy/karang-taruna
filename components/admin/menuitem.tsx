...
export default function MenuItem({ link, toggleSideNavOpen }: { link: LinkType, toggleSideNavOpen: () => void }) {
        ...
        <>
            {link.child ? (
                ...
                onClick={toggleChildMenu}
                    className={clsx(
                        `flex w-full items-center gap-4 py-2.75 px-3 rounded-xl hover:bg-gray-100 ${isOpen && ('border border-gray-200')}`, {
                            'border border-gray-400': pathname.startsWith(link.href)
                        }
                    )}
                >
            ...
            {link.child && isOpen && (
                <div className={`ml-7 mt-1 max-h-fit overflow-hidden transition-all duration-500`}>
                            ...
                            return (
                                <li key={child.name}>
                                    ...
                                    >
                                        ...
                                        <span className="flex-auto">{child.name}</span>
                                    </Link>
                                </li>
                            );
                        })}
